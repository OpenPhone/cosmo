...
func NewDefaultFactoryResolver(
	...
	rabbitMQPubSubBySourceID map[string]pubsub_datasource.RabbitMQPubSub,
	...
	return &DefaultFactoryResolver{
		...
		transportOptions:   transportOptions,
		static:             &staticdatasource.Factory[staticdatasource.Configuration]{},
		pubsub:             pubsub_datasource.NewFactory(ctx, natsPubSubBySourceID, kafkaPubSubBySourceID, rabbitMQPubSubBySourceID),
		log:                log,
		...
		case nodev1.DataSourceKind_PUBSUB:
			...
			for _, eventConfiguration := range in.GetCustomEvents().GetRabbitmq() {
				...
				eventConfigurations = append(eventConfigurations, pubsub_datasource.EventConfiguration{
					...
					Configuration: &pubsub_datasource.RabbitMQEventConfiguration{
						Queues: eventConfiguration.GetQueues(),
					},
				})
			}
